...
class Movie:
    ...
    def __init__(self, id, date, rank):
        ...
        self.highest_in_subtree = (id, rank)
...
class FlixChillManager(AVL):
    ...
    def maintain(self):
        ...
        if not self.left and not self.right:
            self.item.highest_in_subtree = (self.item.id, self.item.rank)

        elif not self.left:
            self.item.highest_in_subtree = max((self.item.id, self.item.rank),self.right.item.highest_in_subtree,
                                               key=lambda x: x[1])
        elif not self.right:
            self.item.highest_in_subtree = max((self.item.id, self.item.rank), self.left.item.highest_in_subtree,
                                               key=lambda x: x[1])
        else:
            self.item.highest_in_subtree = max((self.item.id, self.item.rank), self.left.item.highest_in_subtree,
                                           self.right.item.highest_in_subtree, key=lambda x: x[1])
    ...
    def find_highest(self, s):
        if self.item.key < s:
            if self.right:
                return self.right.find_highest(s)

        if self.item.key == s:
            if self.right:
                return max((self.item.id, self.item.rank), self.right.item.highest_in_subtree, key=lambda x: x[1])
            return (self.item.id, self.item.rank)

        if self.item.key > s:
            if self.left and self.right:
                return max(self.left.find_highest(s), (self.item.id, self.item.rank), self.right.item.highest_in_subtree,
                           key=lambda x: x[1])
            if self.left:
                return max(self.left.find_highest(s), (self.item.id, self.item.rank), key=lambda x: x[1])
            if self.right:
                return max((self.item.id, self.item.rank), self.right.item.highest_in_subtree, key=lambda x: x[1])
    ...
    # time argument: never recurse on both sides, only recurse on the height once
